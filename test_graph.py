...
from dataloader import trainloader, testloader, pretrainloader
...
g = g.to('cpu')
agent = agent.to('cpu')
pre_trainer = NetworkTrainer(pretrainloader, testloader, agent, g, exe)
pre_trainer.fit(5)
trainer = A2CTrainer(trainloader, testloader, agent, g, exe, entropy_coef=0.0)
trainer.fit(5)

# inp = torch.randn(1, 28, 28, 1)
...
#     return int(action.item()) - 1, value.squeeze(), log_prob.squeeze(), entropy.squeeze()

# obs, done = exe.reset_input(inp)
# while not done:
#     action, value, log_prob, entropy = act(obs)
#     obs_, reward, done = exe.step(action)
#     obs = obs_
